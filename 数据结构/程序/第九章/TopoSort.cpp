...
void ExtLGraph<T>::TopoSort(int* order){
 ...
 for(i = 0; i < LGraph<T>::n; i++)
  if(!InDegree[i]){InDegree[i] =top; top =i;}
 for(int i =0; i<LGraph<T>::n; i++){
  if(top ==-1) throw HasCycle;
  else{
   j = top; 
   top = InDegree[top];
   order[i] = j; cout << j << "  ";
   for(p =LGraph<T>::a[j]; p; p=p->nextArc){
    k = p->adjVex;
    InDegree[k]--;
    if(!InDegree[k]){InDegree[k] =top; top =k;}
   }
  }
 }
}
...
void ExtLGraph<T>::CallInDegree(int* InDegree){
 for(int i =0; i <LGraph<T>::n; i++) InDegree[i] =0;
 for(int i =0; i <LGraph<T>::n; i++)
...
int main(void){
 ...
 cout << endl << endl 
      << "逐步建构图9-11的TopoSort(P190)"  << endl << endl;
 int w =10; lg.Insert(0, 2, w); lg.Insert(0, 7, w);  
 lg.Insert(1, 2, w); lg.Insert(1, 3, w); lg.Insert(1, 4, w);
 lg.Insert(2, 3, w);  
 lg.Insert(3, 5, w); lg.Insert(3, 6, w);
 lg.Insert(4, 5, w);  
 lg.Insert(7, 8, w); 
 lg.Insert(8, 6, w);
