...
class SeqQueue{
 public:
  ...
  bool EnQueue(T x){
   if(IsFull()){cout << "Full" << endl; return false;}
   q[(rear +1)% maxSize]=x;
   return true;
  }
  bool DeQueue(){
...
void ExtLGraph<T>::DFS(){
 ...
 for(int i= 0; i <LGraph<T>::n; i++) visited[i] =false;
...
int main(void){
 ExtLGraph<int> lg(7);
 cout << endl << endl 
      << "逐步建构图9-9的DFS序列(P187)"  << endl << endl;
 int w =10; lg.Insert(0, 1, w); 
 lg.Insert(1, 2, w);  lg.Insert(1, 3, w); 
 lg.Insert(2, 0, w);  
 lg.Insert(3, 2, w);  lg.Insert(3, 0, w);
 lg.Insert(4, 1, w);
 lg.Insert(5, 3, w); lg.Insert(5, 4, w);
 lg.Insert(6, 5, w); lg.Insert(6, 4, w);
 cout << endl << "DFS=(";
 lg.DFS();
 cout << ")" << endl;
 cout << endl << "BFS=(";
